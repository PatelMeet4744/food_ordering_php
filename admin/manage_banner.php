...
if (isset($_POST['submit'])) {
	...
	if ($id == '') {
		...
		if ($type != 'image/jpeg' && $type != 'image/png') {
			...
			mysqli_query($con, "insert into banner(image,heading,sub_heading,link,link_txt,order_number,status,added_on) values('$image','$heading','$sub_heading','$link','$link_txt','$order_number',1,'$added_on')");
			redirect('banner.php');
		}
	} else {
		if ($_FILES['image']['type'] == '') {
			...
			if ($type != 'image/jpeg' && $type != 'image/png') {
				...
				$image = rand(111111111, 999999999) . '_' . $_FILES['image']['name'];
				move_uploaded_file($_FILES['image']['tmp_name'], SERVER_BANNER_IMAGE . $image);
				mysqli_query($con, "update banner set image='$image', heading='$heading', sub_heading='$sub_heading', link='$link', link_txt='$link_txt', order_number='$order_number' where id='$id'");
				redirect('banner.php');
			}
		}
	}
		...
		<div class="card">
			<div class="card-body">
				<form class="forms-sample" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
						<label for="exampleInputEmail3" required>Link Text</label>
						<input type="textbox" class="form-control" placeholder="Link Text" name="link_txt" value="<?php echo $link_txt ?>">
					</div>
					<div class="form-group">
